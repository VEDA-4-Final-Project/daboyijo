...
void PrivacyMasker::process(int channel, cv::Mat& image, const std::vector<Detection>& detections) {
    // 1. 낙상 비상 상황 체크 (안전 모드인 경우 블러 처리를 건너뜀)[cite: 1]
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (fall_events_.count(channel)) {
            ...
            if (elapsed < unmask_duration_sec_) {
                return; // 10초 미만이면 마스킹을 하지 않고 원본 영상 송출[cite: 1]
            }
        }
    }

    // 2. 평상시: WiseAI 메타데이터 기반 얼굴 영역 블러 처리[cite: 1]
    for (const auto& d : detections) {
        if (d.type == "Head" || d.type == "Face") {[cite: 1]
            // 정규화 좌표(0.0~1.0)를 입력 이미지의 실제 픽셀 좌표로 변환[cite: 1]
            int x0 = std::max(0, static_cast<int>(d.left * image.cols));[cite: 1]
            int y0 = std::max(0, static_cast<int>(d.top * image.rows));[cite: 1]
            int x1 = std::min(image.cols, static_cast<int>(d.right * image.cols));[cite: 1]
            int y1 = std::min(image.rows, static_cast<int>(d.bottom * image.rows));[cite: 1]

            cv::Rect face_roi(x0, y0, x1 - x0, y1 - y0);

            // 이미지 경계 밖 예외 방지 및 유효성 검사[cite: 1]
            if (face_roi.width > 0 && face_roi.height > 0) {
                cv::GaussianBlur(image(face_roi), image(face_roi), 
                                 cv::Size(blur_kernel_size_, blur_kernel_size_), 20);[cite: 1]
            }
        }
    }
}