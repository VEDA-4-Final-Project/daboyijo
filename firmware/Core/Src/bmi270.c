...
HAL_StatusTypeDef BMI270_Init(void)
{
    ...
    for (uint16_t i = 0; i < 8192; i += 256)
    {
		...
		if (HAL_SPI_Transmit(&hspi2, &reg_init_data, 1, 10) != HAL_OK) return HAL_ERROR;
		if (HAL_SPI_Transmit(&hspi2, (uint8_t*)&bmi270_config_file[i], 256, 100) != HAL_OK) return HAL_ERROR;
...
// 자이로 XYZ축 한번에 읽기 함수. 주소(1B) + 더미(1B) + 데이터(6B)
HAL_StatusTypeDef BMI270_Read_Gyro(BMI270_Data_t *gyro)
{
    ...
    // ±2000dps 범위 설정 기준: 16.384 LSB = 1dps 이므로 나누어 실수 변환
