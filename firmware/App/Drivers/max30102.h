...
/* MODE_CONF 동작값 —— 0x03 = SpO2 모드 (RED + IR 동시 측정).
 *
 * 이 값이 유지되는지가 곧 '센서가 살아 있는가' 다. 전원이 흔들리거나
 * 충격으로 센서가 자체 리셋되면 MODE_CONF 가 0x00 으로 돌아가고,
 * 그 순간 LED 는 꺼지고 FIFO 도 채워지지 않는다.
 * MCU 는 그 사실을 스스로 알 수 없으므로 MAX30102_IsAlive() 로 확인한다. */
#define MAX30102_MODE_CONF_VAL       0x03
...
/* 센서 설정이 살아 있는가 (MODE_CONF 를 되읽어 확인).
 *
 * @return 1 = 정상 동작 중, 0 = 리셋됐거나 통신 불가 → 재초기화 필요
 *
 * 충격·전원 드룹으로 센서만 리셋되면 I2C 버스는 멀쩡한데 센서는 죽어 있다.
 * 그 상태를 구분할 수 있는 유일한 방법이 설정값 되읽기다. */
