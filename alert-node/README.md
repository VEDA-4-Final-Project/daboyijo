...
| 부품 | 무엇인가 | 특이점 |
| --- | --- | --- |
| **WM8960** | 오디오 코덱 칩 | 파이가 I2S 로 보낸 디지털 소리를 아날로그로 바꿔 스피커로 출력. 설정은 I2C |
...
MQTT 없이 패널에 한글 경보를 흘려 보는 데모입니다. hub75 모듈이 올라와 있어야 합니다.
...
(`alert-display.hpp`) 로 분리되어 있어, MQTT 연동은 이벤트 소스만 갈아끼우면 됩니다.

---

## 9. 글꼴

`hub75-font16.h` 는 `genfont.py` 가 나눔고딕Bold 에서 구워내는 **자동 생성 파일**입니다.
