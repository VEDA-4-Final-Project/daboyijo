...
요양원 복도에 걸어 두는 경보기입니다. 서버가 낙상이나 침상 이탈을 확정하면
MQTT 로 명령이 날아오고, 이 노드가 사이렌을 울리면서 LED 패널에 "301호 홍O준
낙상 발생" 같은 문구를 흘립니다. 관제 앱에서 "경보 해제" 를 누를 때까지
소리와 화면이 계속 유지됩니다.
...
경보가 없는 평상시에는 패널에 현재 시각을 띄웁니다.

## 다루는 하드웨어

두 부품의 커널 드라이버를 직접 구현했습니다(프로젝트 필수 요구사항이라 기성
드라이버를 쓰지 않았습니다).

**wm8960** 은 오디오 코덱 칩입니다. 파이가 I2S 로 보낸 디지털 소리를 아날로그로
바꿔 스피커로 내보냅니다. 설정은 I2C 로 합니다.

**HUB75** 는 LED 패널의 배선 규격입니다. 이 패널은 화면을 스스로 기억하지
못합니다. 한 번에 두 줄만 켤 수 있어서, CPU 가 32줄을 쉬지 않고 빠르게 돌아가며
켜야 사람 눈에 정지 화면으로 보입니다. 이 갱신이 잠깐만 끊겨도 화면이 눈에
띄게 떨립니다. 그래서 파이의 CPU 코어 하나를 통째로 이 일에만 씁니다.
...
여기서 **오버레이**(device tree overlay)는 커널에게 "이 GPIO 핀들에 이런 장치가
붙어 있다" 고 알려 주는 설정 조각입니다. 파이는 부팅할 때 이 정보를 읽어
장치 목록을 만들고, 그래야 우리가 만든 드라이버가 자기 장치를 찾아 붙습니다.
드라이버(`.ko`)와 오버레이(`.dtbo`)가 항상 짝으로 따라다니는 이유입니다.

## 처음 한 번 해야 하는 것

새 파이에 올릴 때만 하면 됩니다. **현재 개발용 파이에는 이미 적용돼 있으니**,
아래 두 명령으로 확인해서 값이 나오면 이 절은 건너뛰고 빌드로 갑니다.
...
패널에 필요한 것들이 전부 부팅 시점에 결정되는 값이라, 여기에 넣고 재부팅해야
합니다.
...
네 줄이 각각 왜 필요한지는 이렇습니다.

- `isolcpus=3` 은 리눅스에게 "CPU 3번은 쓰지 마라" 고 알리는 것입니다. 위에서
  말한 화면 갱신을 그 코어가 전담합니다. 다른 작업이 끼어들면 화면이 떨립니다.
  커널 부팅 파라미터라서 재부팅 없이는 못 바꿉니다.
- `gpio=12=op,dh,pu` 는 패널의 OE(출력 활성) 핀을 부팅하자마자 꺼 둡니다. 이게
  없으면 전원이 들어오고 드라이버가 올라오기 전까지 패널이 켜진 채로 있어
  뜨거워집니다.
- `dtparam=spi=off` — SPI 기능이 GPIO 7~11 을 먼저 차지하는데, 그 핀들이 패널
  데이터선입니다.
- `dtparam=audio=off` — 파이 내장 아날로그 오디오가 패널과 같은 PWM 을 씁니다.

### pulseaudio 가 사운드카드를 못 잡게 하기

데스크톱 화면이 뜨는 파이에서만 필요합니다. pulseaudio 가 사운드카드를 먼저
잡아 버리면, 모듈을 다시 올린 직후 5~10초 동안 재생이
`Device or resource busy` 로 실패합니다. lightdm 과 사용자 세션이 각각 띄우기
때문에 두 번 겹치면 10초까지 갑니다.
...
`load.sh` 는 이미 올라와 있으면 건너뛰므로 여러 번 실행해도 괜찮습니다. 앱은
`root` 로 띄워야 합니다 — 패널 밝기를 sysfs 로 바꾸기 때문입니다.
...
토픽 `veda/alarm/control` 로 `AlarmCommand` JSON 이 옵니다. 각 필드가 이렇게
대응됩니다.
...
문구는 노드가 짧게 조립합니다. 64px 화면을 한 바퀴 흘리는 데 문구 길이만큼
시간이 들어서, 바뀌는 정보인 호실을 맨 앞에 두고 나머지는 최소한으로
줄였습니다 (`301호 낙상 발생`).
...
현재 시각을 시:분:초로 띄웁니다. 초가 바뀔 때만 다시 그립니다.
`idle_mode = text` 로 두면 대신 `idle_text` 문구가 뜹니다.

파이에는 시계를 유지하는 배터리가 없습니다. 그래서 부팅 직후의 시각은
`fake-hwclock` 이 복원해 둔 지난번 종료 시각이고, 실제와 몇 시간까지
어긋날 수 있습니다. 겉보기에는 멀쩡한 시각이라 더 헷갈립니다. 틀린 시계를
벽에 거느니 안 거는 게 낫다고 보고, systemd-timesyncd 가 NTP 로 시각을 맞춰
`/run/systemd/timesync/synchronized` 를 만들기 전까지는 `idle_text` 로
물러납니다.
...
패널 쪽은 오버레이만으로 끝나지 않습니다. 위의 부트 설정이 먼저 들어가 있어야
...
`insmod` 와 `rmmod` 를 직접 쓰면 `rmmod` 가 거부됩니다. HDMI 가 없으면 이
패널이 `fb0` 이 되는데, 리눅스 콘솔(fbcon)이 거기에 자동으로 붙어 모듈을
쓰는 중으로 잡아 두기 때문입니다. `make reload` 가 그 연결을 먼저 끊어 줍니다.

패널이 켜진 채로 멈췄을 때는 이 명령으로 끕니다.
...
MQTT 없이 패널에 한글 경보를 흘려 보는 데모입니다. hub75 모듈이 올라와 있어야
합니다.
...
이벤트는 `alert-demo.cpp` 가 난수로 만듭니다. 렌더링은 `AlertDisplay`
